...
public class CsvReader implements Constants
{
	...
	private void addRoom(String line, int number)
	{
		try
		{
			...
			NavigationConnection navigationConnection = getNavigationConnection(Integer
					.parseInt(value[10]));
			String aliases = value[11];

			Room room = new Room(roomNumber, name, function, floor, coordX,
					coordY, radius, doorsX, doorsY, navigationConnection,
					aliases);
			creator.add(room);
		} catch (Exception e)
		{
