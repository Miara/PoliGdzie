...
public class DirectionsTest {
	...
	public void setUp() throws Exception {
    	...
    	/* œci¹gniêcie niepustego drzewa z API, aby omin¹æ null pointer exception */
    	Directions directions = mapper.readValue(new URL(url), Directions.class);
    	 		
		
		directions.getRoutes().get(0).getLegs().get(0).getSteps().get(0).getPolyline().setPoints("_p~iF~ps|U_ulLnnqC_mqNvxq`@");
		/* czyszczenie pozostaloœci */
		
		for(int i = 1; i <= directions.getRoutes().get(0).getLegs().get(0).getSteps().size(); i++) {
	...
	public void testGeneratePolylineFromDirections() {
		...
		assertSame(directions.getRoutes().get(0).getLegs().get(0).getSteps().get(0).getPolyline().decodePolyline(), points);*/
	}
